...
We found \texttt{ifn?} was often used to avoid a \texttt{(fspec :args (cat :x any?))}
spec, which would otherwise use the generator for \texttt{any?} to generatively test
the function value. For common variants on polymorphic functions like
\texttt{map}, \texttt{filter}, and \texttt{reduce}, these semantics are unlikely
to be useful, and we found several examples in our experiments that support this view.
